...
| Worktree release audit | `python skills/autoresearch/scripts/audit_release.py worktree --root .` | Maintainer | Local candidate workspace | Content-free stdout reviewed locally; no report file | None | After every candidate change and again after staging | Block staging completion and every remote action |
| Reachable history and candidate archive | `python skills/autoresearch/scripts/audit_release.py all --root . --ref HEAD` and CI `repository-audit` | Maintainer | Local candidate plus Ubuntu full-history checkout | Content-free stdout locally and GitHub check conclusion | None | Every candidate commit | Stop publication; credential/session findings are never allowlisted |
